...
class kNN:
    ...
    def predict(self, point: np.ndarray, k):
        return self._predict_internal(self.dev, point, k)
    ...
    def k_accuracy(self, k):
        points = self._dev_orig[self.features].to_numpy()
        success = 0
        for i, point in enumerate(points):
    ...
    def _predict_internal(self, data, point: np.ndarray, k):
        ...
        dist_transform = lambda x: self.distance_func(x, self.normalize(point))
